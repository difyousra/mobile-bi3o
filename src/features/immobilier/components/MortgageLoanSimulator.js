...
import {
  ...
  MIN_LOAN_DURATION,
  recommendedApport,
  RECOMMENDED_APPORT_PCT,
} from "../utils/mortgageLoanCalculator";

export default function MortgageLoanSimulator({
  initialPrix = 0,
  compact = false,
  ...
  const [apport, setApport] = useState(() => {
    const n = Number(initialPrix) || 0;
    return n > 0 ? String(recommendedApport(n)) : "";
  });
  const [duree, setDuree] = useState(DEFAULT_LOAN_DURATION);

  useEffect(() => {
    const next = Number(initialPrix) || 0;
    if (!next) return;
    setPrix(String(next));
    setApport(String(recommendedApport(next)));
  }, [initialPrix]);
  ...
  const handlePrixChange = (val) => {
    setPrix(val);
    const n = Number(val);
    setApport(n > 0 ? String(recommendedApport(n)) : "");
  };
      ...
      <View style={styles.field}>
        ...
        <View style={styles.stepper}>
          <TouchableOpacity
            ...
            onPress={() => setDuree((d) => Math.max(MIN_LOAN_DURATION, d - 1))}
          >
...
const styles = StyleSheet.create({
  ...
  resultStrong: { fontWeight: "800", color: colors.textHeading },
  disclaimer: {
    fontSize: 11,
